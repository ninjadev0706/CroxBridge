...
import {
  ...
  fetchPrevFarmUserStakedBalances,
...
export const fetchFarmUserDataAsync = (account) => async (dispatch) => {
  ...
  const userPrevStakedBalances = await fetchPrevFarmUserStakedBalances(account);
  ...
  const arrayOfUserDataObjects = userFarmAllowances.map(
    (farmAllowance, index) => {
      return {
        ...
        prevStakedBalance: userPrevStakedBalances[index],
